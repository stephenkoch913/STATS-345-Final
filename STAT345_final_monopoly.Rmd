...
spaces_landed_on <- rep(0, num_simulations)

for(i in 1:num_simulations) {
  move <- dice()$movement
  if(cur_space+move > 40){
    cur_space = cur_space+ move -40
  }
  else {
    cur_space = cur_space+move
    
  }
  spaces_landed_on[i]= cur_space
}

df =  tibble(spaces_landed_on) 

ggplot(data = df, aes(x = spaces_landed_on)) + geom_histogram(binwidth = 1)
```
