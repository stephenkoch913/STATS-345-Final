...
if(verbose) cat("Player at:", pos,".")
pos <-  n
if(verbose) cat("Player at:", pos,".")}, trap = function(n){if(verbose) cat("Player at:", pos,".")
pos <- 11
if(verbose) cat(" Player now in jail.\n")
jail <- 1}, getout = function(n){jail <- 0
if(verbose) cat("Player free from jail")
pos <- 11 + n
if(verbose) cat("Player now at:", pos, "\n")},
...
chance_function <- function(player, tracking, verbose = FALSE){
draw <- sample(1:15, 1)
if (draw %in% 1:9) {
tracking$increase_count(player$pos)
player$go_2_space_n(chance_card_results[draw])
if(verbose) {
cat("Tally position", player$pos, as.charecter(monoploy_board$title[player$pos]), "\n")
cat("Player now at", chance_deck[draw], "\n")
} }
if (verbose) {
cat("Draw Chance Card ", draw, as.charecter(chance_deck$title[player$pos]), "\n") 
} }


community_function <- function(player, tracking, verbose=FALSE){
draw <- sample(1:16, 1)
if (draw %in% 1:2) {
tracking$increase_count(player$pos)
...
if (verbose) {
cat("Draw Community Chest Card ", draw, as.charecter(community_deck$title[player$pos]), "\n")
} }
...
player$move_n(roll$movement)
} } else if ( !roll$doubles & player$jail >= 1) {
